...
2. `TrajectoryGenerator`
   ...
   Генерирует траекторию пера напрямую по тексту: нормализованные `dx`, `dy` и
   `pen_up`. Текст кодируется TransformerEncoder, а позиции будущей траектории
   монотонно привязываются к символам. Дальше по временной оси работают
   локальные Conv-блоки, поэтому стоимость обучения растёт линейно от длины
   строки и остаётся пригодной для GTX 1660. Loss считается по дельтам,
   накопленной траектории, подъёмам пера, curvature, предсказанной длине,
   pen-aware raster-картинке, bbox и разбросу дельт. Дополнительные shape-loss
   нужны, чтобы модель не могла получить низкий loss простой средней линией.
...
.venv/bin/python main_generate.py --profile cpu_smoke "Тест." --point-length 96 --temperature 0
...
По умолчанию `main_evaluate.py` для генератора берёт длину траектории из
реального примера. Это упрощает диагностику: если даже при правильной длине
строка нечитаемая, значит генератор не научился соответствию `текст -> точки`.
...
.venv/bin/python main_generate.py "Фиксированная длина." --point-length 512
...
параметр нужен только для старых flow-чекпойнтов. `--point-length` задаёт
количество точек траектории вручную; старый `--latent-length` оставлен только
как совместимый алиас. `--temperature` добавляет небольшой шум в `dx/dy`,
поэтому для диагностики лучше ставить `--temperature 0`.
...
2. Если `autoencoder` хороший, но `generator` плохо пишет даже `--split train`,
   значит генератор не научился отображению `текст -> траектория`.
...
В текущей версии генератор обучается как прямой supervised trajectory decoder.
Если генератор был обучен старой flow/latent-regressor версией или версией до
`generator_training_version = 5`, его нужно переобучить:
